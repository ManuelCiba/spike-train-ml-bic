...
def normalize(matrix):
    return stats.zscore(matrix, nan_policy='omit')
...
def community_leading_eigenvector(x):
    ...
    # Simplify the graph (otherwise error for really sparse matrices)
    x = x.simplify()
...
def compute_all_features(list_graph):
    ...
    list_final = [list_assortativity,
                  ...
                  list_eccentricity,
                  list_density,
                  list_reciprocity]# print list_spaning1


    header=['Assortativity',
            'APL',
            'BC',
            'CC',
            'Diameter',
    ...
    return list_final, header

def save_result_to_hd(full_path, measure_list, header):
    # create directory
    result_folder = os.path.dirname(full_path)
    os.makedirs(result_folder, exist_ok=True)

    df_measures = pd.DataFrame(measure_list)
    df_measures = pd.DataFrame.transpose(df_measures)
    df_measures.to_csv(full_path, index=None, header=header)
